...
from karaoke_b3.core.browser_controller import BrowserController
from karaoke_b3.core.obs_controller import OBSController
...
class MainWindow:
    def __init__(self, root, player, processor, bg_manager):
        ...
        # Stato per gestire l'interruttore del Mirror
        ...
        # ============================================================
        # ARCHITETTURA OBS - INIZIALIZZAZIONE CONTROLLER
        # ============================================================
        self.mirror = MirrorController()   # Gestore WebSocket (opzionale)
        self.browser = BrowserController() # Gestore Finestra Video Unica
        self.obs = OBSController()         # Gestore Stati OBS (F10/F11)
        # ============================================================ 

        # 3. NOTA: Chromium NON viene più avviato qui. 
        # Verrà avviato solo quando premerai il tasto MIRROR.
        # ============================================================
    ...
    def on_mirror_click(self):
        """Gestisce l'interruttore per la finestra di ricerca (Cattura OBS)"""
        if not self.mirror_active:
            print("[SISTEMA] Apertura finestra ricerca YouTube...")
            # Usa launch_video per aprire l'unica finestra di ricerca consentita
            self.browser.launch_video("https://www.youtube.com") 
            self.mirror_active = True
            self.lbl_status.config(text="RICERCA: APERTA (Cattura OBS)", fg="#00ff00")
        else:
            print("[SISTEMA] Chiusura finestra ricerca...")
            # Usa close_video per chiudere l'istanza
            self.browser.close_video() 
            self.mirror_active = False
            self.lbl_status.config(text="RICERCA: CHIUSA", fg="#888888")

    # ==========================================================
    # GESTIONE STATI LOGICI (ARCHITETTURA OBS)
    # ==========================================================

    def play_karaoke_video(self, url):
        """
        STATO 2 — VIDEO KARAOKE ON.
        Apre finestra video -> Comando OBS (F10).
        """
        print(f"[SISTEMA] Attivazione STATO 2 - Avvio video: {url}")
        
        # 1. Aprire UNA SOLA finestra video
        self.browser.launch_video(url)
        
        # 2. Comando OBS (F10): switch scena KARAOKE + Fullscreen Projector
        # Ritardo di 1 secondo per permettere alla finestra di apparire prima della cattura
        self.root.after(1000, self.obs.set_stato_karaoke)
        
        self.lbl_status.config(text="VIDEO ATTIVO (SALA ON)", fg="#00ff00")

    def stop_karaoke_video(self):
        """
        STATO 3 — VIDEO KARAOKE OFF.
        Chiude projector (F11) -> Chiude browser -> Ritorno MPV (IDLE).
        """
        print("[SISTEMA] Attivazione STATO 3 - Chiusura video")
        
        # 1. Chiudere il projector OBS e switch scena IDLE (Hotkey F11)
        if hasattr(self, 'obs'):
            self.obs.set_stato_idle()
        
        # 2. Chiudere la finestra video (Chromium)
        if hasattr(self, 'browser'):
            self.browser.close_video()
        
        self.lbl_status.config(text="SISTEMA PRONTO (RADIO IDLE)", fg="cyan")

    def on_closing(self):
        """Spegne tutto e ritorna immediatamente al terminale."""
        print("\n[SISTEMA] Chiusura di sicurezza in corso...")
        try:
            self._save_session()
            
            # --- AGGIUNTA OBBLIGATORIA ---
            # Forza il ritorno allo STATO 3 (IDLE) prima di uscire
            self.stop_karaoke_video()
            # -----------------------------
            
            # Chiude il server WebSocket (se ancora in uso per i comandi)
            if hasattr(self, "mirror"):
                self.mirror.shutdown()

            # Comandi di sistema finali per pulizia profonda
            import os
            os.system("pkill -9 chromium") # Chiude ogni residuo di Chromium
            os.system("fuser -k 8765/tcp") # Libera la porta 8765
            
            # Chiude MPV
            import subprocess
            subprocess.call(["killall", "-9", "mpv"], stderr=subprocess.DEVNULL)
            
            self.root.withdraw()
        except:
            pass
        
        print("[SISTEMA] Risorse liberate. Arrivederci!")
        os._exit(0)

    def on_stop_click(self):
        """Interrompe la riproduzione e ripristina lo STATO 1 (IDLE)"""
        if self._is_switching: return
        print("[SISTEMA] Stop richiesto dall'utente.")
        
        # --- AGGIUNTA OBBLIGATORIA PER ARCHITETTURA OBS ---
        # Riporta immediatamente OBS e il browser in stato IDLE
        self.stop_karaoke_video()
        # --------------------------------------------------
    ...
    def _bind_events(self):
        ...
        # --- ARCHIVIO (Sinistra) ---
        # Usa la funzione in actions per caricare i brani
        self.ui.lsb_files.bind("<Double-Button-1>", self.actions.handle_archive_double_click)
        ...
        ToolTip(self.ui.btn_radio_play, "Carica l'URL o il percorso scritto qui a fianco")

        self.ui.btn_radio_toggle.config(command=self.bg_manager.toggle)
        self.ui.vol_bg.config(command=self.bg_manager.set_volume)

        # --- PATCH: Visualizer e Immagine Radio ---
        self.ui.btn_viz_toggle.config(command=self.toggle_visualizer)
        ToolTip(self.ui.btn_viz_toggle, "Attiva/Disattiva il visualizer grafico (Risparmio CPU AMD)")
        ...
        # --- MIRRORING (INTERRUTTORE APRI/CHIUDI) ---
        self.ui.btn_mirror.config(command=self.toggle_mirror)
        # Aggiornato per riflettere la nuova funzione di ricerca YouTube
        ToolTip(self.ui.btn_mirror, "Apri/Chiudi lo specchio di ricerca YouTube sincronizzato")
    ...
    def toggle_sala(self):
        try:
            ...
            is_min = self.player.sala_player.get_property("window-minimized")
            if is_min == "no":
                self.sala_active = True
    ...
    def _save_session(self):
        """Salva playlist, cronologia e ora anche l'immagine di sfondo radio."""
        f = os.path.expanduser("~/karaoke_clean/tmp/session.json")
        try:
            os.makedirs(os.path.dirname(f), exist_ok=True)
            
            # Recuperiamo il percorso dell'immagine attuale per la persistenza
            current_bg = self.player.viz_manager.bg_image
            
            with open(f, "w") as j: 
                # ORA SALVIAMO SIA LA PLAYLIST CHE LA HISTORY E IL BG RADIO
                json.dump({
                    "timestamp": time.time(), 
                    "playlist": self.playlist_data,
                    "history": self.play_history,
                    "radio_bg_image": current_bg  # <--- AGGIUNTA PER PERSISTENZA
                }, j)
        except Exception as e: 
    ...
    def toggle_mirror(self):
        """Prepara l'interfaccia e lancia il toggle in modo asincrono."""
        # Disabilita subito il bottone per evitare clic multipli accidentali
        try:
            self.ui.btn_mirror.config(state="disabled")
        except:
            pass

        # Esegui il toggle con un leggero ritardo per non bloccare il thread della GUI
        self.root.after(50, self._toggle_mirror_safe)

    def _toggle_mirror_safe(self):
        """Esegue il toggle del mirror proteggendo l'integrità dell'interfaccia."""
        try:
            # 1. Forza lo stato disabilitato durante l'elaborazione
            self.ui.btn_mirror.config(state="disabled")
            self.root.update_idletasks() 

            # 2. Chiama la logica Toggle definita in MainWindow/Actions
            self.on_mirror_click()

        except Exception as e:
            print(f"[ERRORE UI] Fallimento durante il toggle mirror: {e}")
        
        finally:
            # 3. Riabilita il bottone in ogni caso (anche se avviene un errore)
            try:
                self.ui.btn_mirror.config(state="normal")
            except:
                pass

    # ==========================================================
    # LOGICA VISUALIZER E BACKGROUND RADIO
    # ==========================================================
    def toggle_visualizer(self):
        """Attiva/Disattiva la grafica della radio per risparmiare risorse sulla GPU AMD."""
        v = self.player.viz_manager
        v.enabled = not v.enabled
        
        # Aggiornamento estetico del tasto
        ...
        # Se la radio è in esecuzione, ricarica per applicare il cambio
        if hasattr(self, 'bg_manager') and self.bg_manager.is_playing and not self._is_switching:
            self.bg_manager.play_current()

    def load_radio_image(self):
        """Carica un'immagine locale da usare come sfondo radio (Modalità Risparmio CPU)."""
        ...
        if p:
            ...
            print(f"[SISTEMA] Background radio impostato: {os.path.basename(p)}")
            
            # Se il visualizer è spento, ricarichiamo per mostrare l'immagine subito
            if not self.player.viz_manager.enabled and self.bg_manager.is_playing:
                self.bg_manager.play_current()

...
class MediaLogic:
    ...
    def _download_with_progress(self, url, out_path, item, retry=True):
        """Esegue yt-dlp con sistema di Auto-Aggiornamento in caso di errore."""
        cmd = [
        ...
        print(f"[DOWNLOAD] Avvio tentativo per: {url}")
        ...
        for line in proc.stdout:
            ...
            # FIX: L'allarme scatta SOLO se c'è un vero errore critico, non per i WARNING aggirati!
            if line.startswith("ERROR:") and ("sign in to confirm" in line.lower() or "bot" in line.lower()):
                error_detected = True
        ...
        # Se il programma chiude con successo (returncode 0) e non ci sono errori bot confermati, passiamo!
        if proc.returncode != 0 or error_detected:
            if retry:
                print("[AUTO-FIX] Blocco di YouTube rilevato. Tento l'aggiornamento forzato di yt-dlp...")
                item["phase"] = "UPDATING YTDL"
                if getattr(self, 'mw', None):
                    self.mw.root.after(0, self.mw.refresh_tree)
                ...
                try:
                    ...
                    print("[AUTO-FIX] Aggiornamento completato con successo. Riprovo il download...")
                    return self._download_with_progress(url, out_path, item, retry=False)
                except Exception as e:
                    print(f"[AUTO-FIX] Errore critico durante l'aggiornamento: {e}")
            
            raise RuntimeError(f"yt-dlp ha restituito errore {proc.returncode} o blocco bot.")
